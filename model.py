...
class Model(nn.Module):
    def __init__(self,opt_path:str):
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.opt['train']["learning_rate"])
        ...
        for epoch in range(total_epochs):
            for i, (inputs, labels) in enumerate(self.dataloader):
                ...
                if count % self.opt['train']['print_freq'] == 0:
                    ...
                    print(f"epoch {epoch+1}/{total_epochs} , iter {count} train loss: {loss.item()} , val loss: {loss_valid}")
    ...
    def load_model(self, file_path):
        ...
        print(f'Model state_dict loaded from {file_path}')